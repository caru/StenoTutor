...
public void showTextInfo(Stroke stroke) {
  ...
  text(isLessonStarted ? (int) (typedWords / (getElapsedTime() / 60000.0f)) : 0, wpmX, wpmY);
...
// Update current min and max penalty limits
public void updatePenaltyLimits() {
...
private class WordStats {
  ...
  public WordStats() {
    typeTime.add((long) 60000.0f / wordStartAvgWpm);
  ...
  public float getAvgWpm() {
    ...
    if (typeTime.size() > 0) {
      ...
      return wordAvgSamples * 1.0f / (totalTime / 60000.0f);
    } else {
      return 1.0f;
    }
  }
